...
const BUDGET_STEPS: Array<{ icon: IconName; title: string; text: string }> = [
  {
    icon: 'calendar',
...
const SAVINGS_STEPS: Array<{ icon: IconName; title: string; text: string }> = [
  {
    icon: 'target',
    ...
    <div className="modal-overlay fixed inset-0 z-[80] flex items-center justify-center p-4">
      <div className="w-full max-w-sm glass-card rounded-3xl overflow-hidden">
        ...
        <div className="grad-lime-strong px-5 pt-5 pb-3 text-white">
          <div className="mb-1"><Icon name={current.icon} size={32} strokeWidth={1.75} /></div>
            ...
            {steps.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition ${
                  i === step ? 'bg-accent-emerald scale-110' : 'bg-neutral-300 dark:bg-neutral-700'
                }`}
              />
            ))}
          </div>
